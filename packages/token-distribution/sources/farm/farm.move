...
module token_distribution::farm {
    ...
    public fun get_unlock_per_second<T>(farm: &Farm<T>): u64 {
        td::unlock_per_second(&farm.td)
    }

    public fun get_unlock_start_ts_sec<T>(farm: &Farm<T>): u64 {
        td::unlock_start_ts_sec(&farm.td)
    }
 
    public fun get_final_unlock_ts_sec<T>(farm: &Farm<T>): u64 {
        td::final_unlock_ts_sec(&farm.td)
    }

    public fun get_remaining_unlock<T>(farm: &Farm<T>, clock: &Clock): u64 {
        td::remaining_unlock(&farm.td, clock)
    }

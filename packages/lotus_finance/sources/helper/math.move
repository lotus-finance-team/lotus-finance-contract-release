...
module lotus_finance::lotus_math{
    use sui::math;
    use std::u64;
    ...
    const EIntegerOverflow: u64 = 1;

    // Scale a u64 value to float representation with 9 fixed decimal points
    public(package) fun scale_to_float(x: u64, decimal: u64): u128 {
        (x as u128 * FLOAT_SCALING_U128 / (math::pow(10, decimal as u8) as u128)) as u128
    }

    public(package) fun scale_from_float(x: u128, decimal: u64): u64 {
        let rst = (x as u128 * (math::pow(10, decimal as u8) as u128) / FLOAT_SCALING_U128) as u128;
        assert!(rst < std::u64::max_value!() as u128, EIntegerOverflow);
        rst as u64
    }
    ...
    /// Multiply and Divide three integers.
    public(package) fun mul_div_u64(x: u64, y: u64, z: u64): u64 {
        let rst = ((x as u128 )* (y as u128) / (z as u128));
        assert!(rst < std::u64::max_value!() as u128, EIntegerOverflow);
        rst as u64
    }

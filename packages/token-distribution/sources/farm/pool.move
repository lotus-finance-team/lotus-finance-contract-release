...
module token_distribution::pool {
    ...
    public fun new_top_up_ticket_with_farm_key(farm_key: &mut FarmMemberKey): TopUpTicket {
        TopUpTicket {
    ...
    public fun destroy_top_up_ticket_with_farm_key(farm_key: &mut FarmMemberKey, ticket: TopUpTicket) {
        let TopUpTicket { withdraw_all_ticket } = ticket;
    ...
    public fun top_up_with_acc<T>(
        farm: &mut Farm<T>, acc: &mut AccumulationDistributor, ticket: &mut TopUpTicket, clock: &Clock
    ) {
        let balance = farm::member_withdraw_all_with_ticket(farm, &mut ticket.withdraw_all_ticket, clock);
